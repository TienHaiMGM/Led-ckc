...
const NeonBox = ({ letter, color }: { letter: string; color: string }) => {
  return (
    <motion.div
      className={`flex h-32 w-60 -rotate-2 animate-flicker cursor-pointer items-center justify-center rounded-md border-4 p-1 text-8xl font-bold hover:scale-105 xl:h-80 xl:w-80 xl:text-[12rem] border-[${color}] bg-black transition-all duration-300 ease-in-out`}
      style={{
