...
const ProductEditorNew: React.FC<EditorProps> = ({
  EditorContent,
  onPreview,
}) => {
  const {
  ...
  const filteredAndSortedProducts = products
    .filter((products) => {
      ...
      const matchesSearch =
        products.title.toLowerCase().includes(searchLower) ||
        products.description?.toLowerCase().includes(searchLower) ||
        products.content?.toLowerCase().includes(searchLower);
      ...
      <div className="overflow-x-auto rounded-lg bg-white shadow-lg">
        <table className="min-w-full divide-y divide-gray-200">
          ...
          <tbody className="divide-y divide-gray-200 bg-white">
            {filteredAndSortedProducts.map((products) => (
              <tr key={products.id}>
                ...
                <td className="whitespace-nowrap px-6 py-4">
                  <div className="flex space-x-2">
                    ...
                    <button
                      onClick={() => handleDelete(products.id!, products)}
